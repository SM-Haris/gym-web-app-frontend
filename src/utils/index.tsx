...
const disabled7DaysDate: DatePickerProps["disabledDate"] = (
  ...
  const convertTimeToDecimal = (data:{hrs: number, mins: number}): number => {
    if (data.hrs < 0 || data.mins < 0 || data.mins >= 60) {
      message.error("Invalid input. Hours must be non-negative, and minutes must be between 0 and 59.");
    }
  ...
  const convertDecimalToTime = (decimalTime: number): { hrs: number; mins: number } => {
    if (decimalTime < 0) {
